...
import logging
#from TP2.sample_mqttpubsub import MQTT_SERVER
...
class CommModule(Thread):
    ...
    topicPub = "1R1/014/shutter"
    topicSub = "1R1/014/shutter/command"
    ...
    def __init__(self, mqtt_user, mqtt_passwd, mqtt_topics, list_shutter, *args, **kwargs ):
        super().__init__()
        ...
        self.list_shutter=list_shutter
    ...
    def run( self ):
        ...
        # start connection
        log.info("start MQTT connection to '%s:%d' ..." % (self.mqtt_server,self.mqtt_port))
        #self._connection.connect( host=self.mqtt_server, port=self.mqtt_port, keepalive=settings.MQTT_KEEP_ALIVE )
        ...
        self._connection.connect(self.mqtt_server, self.mqtt_port, 60)
        self._connection.loop_forever()
        # shutdown module
    ...
    def _on_connect(self, client, userdata, flags, rc):
        ...
        try:
            ...
            #    log.debug("subscribing to " + str(self.topicSub))
            #    self._connection.subscribe( self.topicSub )   # QoS=0 default
            log.debug("subscribing to " + str(self.topicSub))
            self._connection.subscribe( self.topicSub )   # QoS=0 default
            log.debug("subscribing to " + str(self.topicPub))
            self._connection.subscribe( self.topicPub )   # QoS=0 default
    ...
    def pubStatus(self, shut):
        payload = { } 
        payload['unitID'] = str(shut.getID())
        payload['status'] = shut.getStatut()
        payload['order'] = shut.getCmdEnCours()
        self.send_message(self.topicPub, payload)
    ...
    def _on_message(self, client, userdata, msg):
        ...
        if(msg.topic==self.topicPub):
            log.warning("Volet : %s, statut : %s, order : %s" % (payload['unitID'], payload['status'], payload['order'])) 

        if(msg.topic==self.topicSub):
            print("Le topic est :" + msg.topic)
            if(payload['dest']=='all'):
                for shut in self.list_shutter :
                    if(payload['order']=='up'):
                        shut.fctUp(self)
                ...
                for shut in self.list_shutter :
                    if(int(shut.getID())==int(payload['dest'])):
                        if(payload['order']=='up'):
                            shut.fctUp(self)
                        elif(payload['order']=='down'):
                            shut.fctDown(self)
