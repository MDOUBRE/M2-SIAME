...
void init(){
	// output 
	GPIOA_MODER = SET_BITS(GPIOA_MODER, LED_BREAD*2, 2, 0b01);
	GPIOA_OTYPER &= ~(1<<LED_BREAD);
	GPIOA_PUPDR = SET_BITS(GPIOA_PUPDR, LED_BREAD*2, 2, 0b01);
}

void handle_TIM4() {
	TIM4_ARR = HALF_PERIOD;
    if(un_sur_deux!=1){    
        if((GPIOA_ODR & (1 << LED_BREAD))==0){
            GPIOA_BSRR = 1 << LED_BREAD;
        }
        else{
            GPIOA_BSRR = 1 << (LED_BREAD+16);
        }
        un_sur_deux+=1;
    }
    else{
